...
##### execute if score #last kit_tick_cd matches 0 run tellraw @s[tag=kitcd,scores={kitcd=1..}] [{"text":"","color":"gold"},{"score":{"name":"@s","objective":"kitcd"},"color":"red"},{"text":" of ","color":"gold"},{"text":"23600","color":"yellow"},{"text":" seconds until you can use Kit again!","color":"gold"}]
#
...
execute store result score @s kit_cdc run scoreboard players operation #last 23600 -= @s kit_cdc
